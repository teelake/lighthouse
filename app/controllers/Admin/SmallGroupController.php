...
{
    public function index() { $this->requireEditor(); $this->render('admin/small-groups/index', ['groups' => (new SmallGroup())->findAll([], 'sort_order ASC'), 'pageHeading' => 'Small Groups', 'currentPage' => 'small-groups']); }
    public function create() { $this->requireEditor(); $this->render('admin/small-groups/form', ['group' => null, 'pageHeading' => 'Add Small Group', 'currentPage' => 'small-groups']); }
    ...
    public function edit() { $this->requireEditor(); $g = (new SmallGroup())->find($this->params['id'] ?? 0); if (!$g) throw new \Exception('Not found', 404); $this->render('admin/small-groups/form', ['group' => $g, 'pageHeading' => 'Edit Small Group', 'currentPage' => 'small-groups']); }
