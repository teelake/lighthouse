...
{
    private const SOURCES_BY_TYPE = [
        'video' => ['youtube', 'vimeo', 'upload', 'external'],
        'audio' => ['upload', 'external'],
        'teaching' => ['youtube', 'vimeo', 'upload', 'external'],
    ];
    ...
    {
        $this->requireEditor();
        $mediaType = $this->post('media_type', 'video');
        $source = $this->getValidSource($mediaType, $this->post('source', 'youtube'));
    ...
    public function update()
    {
        ...
        (new Media())->update($id, [
            ...
            'media_type' => $mediaType, 'source' => $source,
            'embed_url' => $this->post('embed_url'), 'published_at' => $this->post('published_at') ?: null
        ]);
        $this->redirectAdmin('media');
    }
    ...
    private function getValidSource(string $mediaType, string $source): string
    {
        $allowed = self::SOURCES_BY_TYPE[$mediaType] ?? self::SOURCES_BY_TYPE['video'];
        return in_array($source, $allowed, true) ? $source : $allowed[0];
    }
}
