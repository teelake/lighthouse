...
{
    public function index() { $this->requireEditor(); $this->render('admin/jobs/index', ['jobs' => (new Job())->findAll([], 'sort_order ASC')]); }
    public function create() { $this->requireEditor(); $this->render('admin/jobs/form', ['job' => null]); }
    ...
    public function edit() { $this->requireEditor(); $j = (new Job())->find($this->params['id'] ?? 0); if (!$j) throw new \Exception('Not found', 404); $this->render('admin/jobs/form', ['job' => $j]); }
